...
export function diffChildren(
  ...
  */
  ...
  // oldParentVNode._children 或者空数组, 空数组时 _children属性为undefined
    ...
    childVNode = newChildren[i] = coerceToVNode(newChildren[i])
      ...
      /**
       * 寻找旧children中与新vnode判断相等的vnode并删除
      */
			if (
				oldVNode === null ||
				(oldVNode &&
      ...
      // diff将当前的dom更新，此操作会递归遍历所有的子dom （此时尚未append到父dom中）
			newDom = diff(
				parentDom,
				childVNode,
				oldVNode,
          ...
          // 没有oldDom ，或者oldDom的父dom跟形参parentDom不同，此时给parentDom中添加生成的newDom
          // 只有在初始化非render dom的时候，parentNode !== parentDom, 此时按照次序填入dom即可
 ...
 * map中将 基本类型（number, bool）转成vnode
